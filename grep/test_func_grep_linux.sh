...
testing()
{
    ...
    grep $t > test_sys_grep.txt
    DIFF_RES="$(diff -s test_s21_grep.txt test_sys_grep.txt)"
    (( COUNTER++ ))
    if [ "$DIFF_RES" = "Files test_s21_grep.txt and test_sys_grep.txt are identical" ];
    then
      (( SUCCESS++ ))
    ...
    rm -f test_s21_grep.txt test_sys_grep.txt
}
